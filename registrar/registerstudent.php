                        ...
                        <div class="card-body">
                            ...
                            while ($row3 = mysqli_fetch_array($select)) {
                                ...
                                $select1 = mysqli_query($conn,"select * from department where faculity='$faculity' ORDER BY faculity DESC");
                                    ...
                                    ?>
                                    <?php 
                                    echo'<a id="lists" class="text-info" href="registerstudent.php?deptid=' . $departmentid . '">' . strtoupper($departmentname) . '</a> <br>';
                                }
                                ?>  
                                <?php
                            }
                    ...
                    if (isset($_GET['deptid'])) {
                        ...
                        while ($row = mysqli_fetch_array($query1)) {
                            ...
                            <div class="card border-info">
                                <div class="card-header bg-info">LIST of student DEPARTMENT OF  <font color="red"><?php echo strtoupper($department); ?></font></div>
                                <div class="card-body">
                                    <div class=" text-right"> 
                                        <?php echo'<a class="btn btn-success" href="registerstudent1.php?add=' . $departmentid . '">
                                        <i class="fa fa-1x fa-plus text-dark"></i> Add more student</a>'; ?> 
                                    </div>
                                    <?php
                                    $select = mysqli_query($conn,"select * from student where departmentid='$departmentid' ORDER BY firstname ASC");
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                ...
                                                while ($row2 = mysqli_fetch_array($select)) {
                                                    ...
                                                    echo '<tr>';
                                                    echo '<td>' . strtoupper($full) . '</td><td>' . $row2['email'] . '</td><td>' . $row2['sex'] . '</td><td>' .$row2['enroll'] . '</td><td>' .$row2['year'] . '</td><td>' . $row2['semister'] . '</td></tr>';
                                                }
                                                ?>
                                            </tbody></table></div></div>
                                <?php
                            } else {
                                echo 'THERE IS NO STUDENT REGISTER TO THESE DEPARTMENT';
                            }
                        }
                    } 
                    ?>
